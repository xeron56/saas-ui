...
import {
  createRetailSetupRecord,
  deleteRetailSetupRecord,
  listRetailSetupRecords,
  updateRetailSetupRecord,
} from './service';
import type { RetailSetupKind, RetailSetupRecord } from './types';
...
const setupName = (record?: RetailSetupRecord) =>
  record?.name ??
  record?.label ??
  record?.unitName ??
  record?.brandName ??
...
const setupStatus = (record?: RetailSetupRecord) =>
  record?.active === true || record?.status === true || record?.status === 1;

const statusTag = (record?: RetailSetupRecord) => (
...
const subVatIDs = (record?: RetailSetupRecord) =>
  (record?.sub_vat ?? []).map((item) => item.id).filter(Boolean);

const shelfIDs = (record?: RetailSetupRecord) =>
  (record?.shelves ?? []).map((item) => item.id).filter(Boolean);

const branchOptionLabel = (record?: RetailSetupRecord) =>
  record?.name ??
  record?.label ??
  record?.branch?.name ??
  record?.branch?.label ??
...
const matchesStatus = (record: RetailSetupRecord, status?: string) => {
  if (!status) {
    return true;
  }
...
const filterRecords = (items: RetailSetupRecord[], keyword?: string, status?: string) => {
...
const RetailSetupTable: React.FC<{ config: SetupConfig }> = ({ config }) => {
  const intl = useIntl();
  const actionRef = useRef<ActionType>();
  const [form] = Form.useForm();
  const [modalOpen, setModalOpen] = useState(false);
  const [editing, setEditing] = useState<RetailSetupRecord>();
  const [submitting, setSubmitting] = useState(false);
  const [vatOptions, setVatOptions] = useState<RetailSetupRecord[]>([]);
  const [shelfOptions, setShelfOptions] = useState<RetailSetupRecord[]>([]);
  const [branchOptions, setBranchOptions] = useState<RetailSetupRecord[]>([]);

  const loadAuxiliary = async () => {
    if (config.vat) {
      const resp = await listRetailSetupRecords('vats', { type: 'single' });
      setVatOptions(resp.data ?? []);
    }
    if (config.rack) {
      const resp = await listRetailSetupRecords('shelves');
      setShelfOptions(resp.data ?? []);
    }
    if (config.warehouse || config.paymentType) {
      const resp = await listRetailSetupRecords('branches', { status: 'active' });
  ...
  const openEditor = (record?: RetailSetupRecord) => {
    setEditing(record);
  ...
  const submit = async () => {
    ...
    try {
      ...
      const resp = editing
        ? await updateRetailSetupRecord(config.kind, editing.id, payload)
        : await createRetailSetupRecord(config.kind, payload);
  ...
  const columns: ProColumns<RetailSetupRecord>[] = [
    {
      title: 'Name',
      dataIndex: 'name',
      ...
      ? [
          {
            ...
            dataIndex: 'code',
            search: false,
          } as ProColumns<RetailSetupRecord>,
        ]
      : []),
    ...(config.vat
      ? [
      ...
      ? [
          {
            ...
            search: false,
            render: (_, record) =>
              branchOptionLabel(record.branch as RetailSetupRecord) || record.branch_id || '-',
          } as ProColumns<RetailSetupRecord>,
          {
            title: 'Opening',
            ...
            <Button
              ...
              onClick={() => {
                Modal.confirm({
                  ...
                  onOk: async () => {
                    const resp = await deleteRetailSetupRecord(config.kind, record.id);
  ...
  return (
    <>
      <ProTable<RetailSetupRecord>
        actionRef={actionRef}
        rowKey="id"
        columns={columns}
        search={{ labelWidth: 72 }}
        request={async (params) => {
          const resp = await listRetailSetupRecords(config.kind, {
...
const RetailSetupPage: React.FC = () => {
  const intl = useIntl();
  ...
  return (
    <PageContainer
      title={intl.formatMessage({ id: 'retail.setup', defaultMessage: 'Retail Setup' })}
    >
      <Tabs
        items={configs.map((config) => ({
          key: config.kind,
          label: config.label,
          children: <RetailSetupTable config={config} />,
        }))}
      />
    </PageContainer>
  );
...
export default RetailSetupPage;
