...
const DueCollectionEditor: React.FC<DueCollectionEditorProps> = ({
  ...
  return (
    <Drawer
      width={980}
      open={open}
      title={t('retail.dueCollection.collect', 'Collect due')}
      onClose={onClose}
      destroyOnClose
      extra={
        <Button
          ...
          onClick={() => form.submit()}
        >
          {t('retail.dueCollection.post', 'Post payment')}
        </Button>
      }
    >
      ...
      >
        <Form.Item name="mode" label={t('retail.dueCollection.mode', 'Settlement type')}>
          <Segmented
            options={[
              { label: t('retail.dueCollection.customer', 'Customer'), value: 'customer' },
              { label: t('retail.dueCollection.supplier', 'Supplier'), value: 'supplier' },
              { label: t('retail.dueCollection.walkIn', 'Walk-in sale'), value: 'walk_in' },
            ]}
            onChange={(value) => {
              form.setFieldsValue({
        ...
        <Divider orientation="left">{t('retail.dueCollection.target', 'Due target')}</Divider>
        <Row gutter={12}>
          {mode !== 'walk_in' && (
            <Col xs={24} md={9}>
              <Form.Item
                name="partner_id"
                label={t('retail.dueCollection.party', 'Customer or supplier')}
                rules={[{ required: true }]}
              >
                <Select
                  showSearch
                  ...
                  placeholder={t('retail.dueCollection.selectParty', 'Select account')}
                />
              </Form.Item>
            </Col>
          )}
          <Col xs={24} md={mode === 'walk_in' ? 12 : 9}>
            <Form.Item
              ...
              label={
                mode === 'walk_in'
                  ? t('retail.dueCollection.invoiceRequired', 'Walk-in invoice')
                  : t('retail.dueCollection.invoiceOptional', 'Invoice allocation')
              }
            ...
            >
              <Select
                ...
                placeholder={t('retail.dueCollection.selectInvoice', 'Select due invoice')}
              />
            </Form.Item>
          </Col>
          <Col xs={12} md={3}>
            <Form.Item name="settled_at" label={t('retail.dueCollection.date', 'Payment date')}>
              <DatePicker style={{ width: '100%' }} />
            </Form.Item>
          </Col>
          <Col xs={12} md={3}>
            <Form.Item name="branch_ref" label={t('retail.dueCollection.branch', 'Branch ref')}>
              <Input maxLength={128} />
            </Form.Item>
          </Col>
        </Row>
          ...
          {(fields, { add, remove }) => (
            <Space direction="vertical" style={{ width: '100%' }} size={10}>
                ...
                <Row gutter={8} key={field.key} align="middle">
                  <Col xs={24} md={5}>
                    <Form.Item
                      ...
                      label={t('retail.dueCollection.paymentMethod', 'Payment method')}
                    >
                      <Select
                        options={[
                  ...
                  <Col xs={12} md={4}>
                    <Form.Item
                      ...
                      label={t('retail.dueCollection.amount', 'Amount')}
                      rules={[{ required: true }]}
                    >
                    ...
                    tenders?.[field.name]?.tender_kind !== 'cheque' && (
                      <Col xs={24} md={5}>
                        <Form.Item
                          ...
                          label={t('retail.report.paymentType', 'Payment type')}
                        >
                          <Select
                            allowClear
                            showSearch
                  ...
                  <Col xs={12} md={6}>
                    <Form.Item
                      ...
                      label={t('retail.dueCollection.referenceNo', 'Reference no.')}
                    >
                      <Input maxLength={128} />
                    </Form.Item>
                  </Col>
                  <Col xs={18} md={6}>
                    <Form.Item
                      ...
                      label={t('retail.dueCollection.chequeNo', 'Cheque no.')}
                    >
                      <Input maxLength={128} />
                    </Form.Item>
                  </Col>
              ...
              <Button icon={<PlusOutlined />} onClick={() => add({ tender_kind: 'cash' })}>
                {t('retail.dueCollection.addPayment', 'Add payment method')}
              </Button>
            </Space>
          )}
        </Form.List>

        <Divider orientation="left">{t('retail.dueCollection.note', 'Note')}</Divider>
        <Form.Item name="note">
