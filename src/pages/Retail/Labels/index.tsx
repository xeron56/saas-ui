...
const BarcodeSvg: React.FC<{ value: string }> = ({ value }) => {
  ...
  return (
    <svg className="retail-label-barcode" viewBox={`0 0 ${x} 42`} preserveAspectRatio="none">
      {bars.map((bar, index) => (
...
const ProductLabelsPage: React.FC = () => {
  ...
  const columns: ProColumns<CatalogItem>[] = [
    {
      title: t('retail.label.search', 'Search'),
      dataIndex: 'search',
      hideInTable: true,
      valueType: 'text',
    },
    {
      title: t('retail.label.type', 'Type'),
      dataIndex: 'item_kind',
      valueEnum: {
        single: { text: t('retail.label.single', 'Single') },
        variant: { text: t('retail.label.variant', 'Variant') },
        bundle: { text: t('retail.label.bundle', 'Bundle') },
      },
      render: (_, record) => {
    ...
    {
      title: t('retail.label.name', 'Name'),
      dataIndex: 'display_name',
    },
    {
      title: t('retail.label.code', 'Code'),
      dataIndex: 'item_code',
      search: false,
      width: 140,
    },
    {
      title: t('retail.label.barcode', 'Barcode'),
      dataIndex: 'barcode',
      search: false,
      width: 160,
    },
    {
      title: t('retail.label.salePrice', 'Sale'),
    ...
    {
      title: t('retail.label.stock', 'Stock'),
  ...
  return (
    <PageContainer title={t('retail.labels', 'Print Labels')}>
      <ProTable<CatalogItem>
        ...
        toolBarRender={() => [
          <Space key="quantity">
            <span>{t('retail.label.quantity', 'Qty')}</span>
            <InputNumber
              min={1}
              max={100}
              value={labelQuantity}
          ...
          <Select
            ...
            options={[
              { label: t('retail.label.columns2', '2 columns'), value: 2 },
              { label: t('retail.label.columns3', '3 columns'), value: 3 },
              { label: t('retail.label.columns4', '4 columns'), value: 4 },
            ]}
          />,
          <Space key="name">
            <span>{t('retail.label.name', 'Name')}</span>
            <Switch checked={showName} onChange={setShowName} />
          </Space>,
          <Space key="price">
            <span>{t('retail.label.price', 'Price')}</span>
          ...
          >
            {t('retail.label.print', 'Print')}
          </Button>,
        ]}
      />

      <div className="retail-label-print-zone">
        <div className="retail-label-sheet" style={sheetStyle}>
          {printableLabels.map(({ item, key }) => {
            const value = barcodeValue(item);
            return (
              <div className="retail-label-card" key={key}>
                {showName && <div className="retail-label-name">{item.display_name}</div>}
                <BarcodeSvg value={value} />
                <div className="retail-label-code">{value || item.item_code || item.id}</div>
                {showPrice && <div className="retail-label-price">{money(item.sale_price)}</div>}
              </div>
            );
          })}
        </div>
