...
const SettingsPage: React.FC = () => {
  ...
  const saveSettings = async () => {
    ...
    try {
      ...
      setSettings(saved);
      message.success(t('product.settings.saved', 'Settings saved'));
    } finally {
      setSaving(false);
    }
  };
    ...
    <PageContainer>
      <div style={sectionStyle}>
        <Space size={12} wrap>
          ...
          <Button type="primary" icon={<SaveOutlined />} loading={saving} onClick={saveSettings}>
            {t('product.settings.save', 'Save settings')}
          </Button>
          <Tag icon={<SettingOutlined />}>
            {t('product.settings.scope', 'Tenant retail preferences')}
          </Tag>
        </Space>
      </div>
      ...
      <div style={sectionStyle}>
        <div style={sectionHeaderStyle}>
          <Space>
            <FileTextOutlined />
            <strong>{t('product.settings.invoice', 'Invoice')}</strong>
          </Space>
        </div>
        <Row gutter={[16, 12]}>
          <Col xs={24} md={12}>
            <div style={fieldStyle}>
              <span style={labelStyle}>{t('product.settings.invoiceSize', 'Invoice size')}</span>
              <Segmented
          ...
          <Col xs={24} md={12}>
            <div style={fieldStyle}>
              <span style={labelStyle}>
                {t('product.settings.invoiceLanguage', 'Invoice language')}
              </span>
              <Input
          ...
          <Col xs={24} md={12}>
            <div style={fieldStyle}>
              <span style={labelStyle}>{t('product.settings.currency', 'Currency format')}</span>
              <Segmented
          ...
          <Col xs={24} md={12}>
            <div style={fieldStyle}>
              <span style={labelStyle}>{t('product.settings.rounding', 'Sale rounding')}</span>
      ...
      <div style={sectionStyle}>
        <div style={sectionHeaderStyle}>
          <Space>
            <GlobalOutlined />
            <strong>{t('product.settings.receiptCopy', 'Receipt copy')}</strong>
          </Space>
        </div>
        <Row gutter={[16, 12]}>
      ...
      <div style={sectionStyle}>
        <div style={sectionHeaderStyle}>
          <Space>
            <PictureOutlined />
            <strong>{t('product.settings.branding', 'Branding refs')}</strong>
          </Space>
        </div>
        <Row gutter={[16, 12]}>
      ...
      <div style={sectionStyle}>
        <div style={sectionHeaderStyle}>
          <Space>
            <AppstoreOutlined />
            <strong>{t('product.settings.productModules', 'Product modules')}</strong>
          </Space>
          <Select
            style={{ width: 160 }}
