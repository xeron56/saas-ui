...
const TransferPage: React.FC = () => {
  ...
  const lotOptions = useMemo(
    () =>
      stockLots.map((lot) => ({
        ...
        } · ${lot.batch_code || t('product.transfer.openBatch', 'open batch')} · ${money(
          lot.quantity,
        )}`,
      })),
    [stockLots, t],
  ...
  const handleSubmit = async (values: TransferFormValues) => {
    ...
    try {
      ...
      message.success(t('product.transfer.created', 'Transfer posted'));
      setOpen(false);
  ...
  const lineColumns: ColumnsType<RetailTransferLine> = [
    {
      title: t('product.transfer.item', 'Item'),
      dataIndex: 'item_name',
      width: 220,
      render: (value: string, record) => value || record.item_code || record.item_id,
    },
    {
      title: t('product.transfer.batch', 'Batch'),
      dataIndex: 'batch_code',
      width: 130,
    },
    {
      title: t('product.transfer.quantity', 'Quantity'),
      dataIndex: 'quantity',
      align: 'right',
      width: 110,
    ...
    {
      title: t('product.transfer.unitCost', 'Unit cost'),
      dataIndex: 'unit_cost',
      align: 'right',
      width: 120,
      render: (value: number) => money(value),
    ...
    {
      title: t('product.transfer.lineCost', 'Line cost'),
      dataIndex: 'line_cost',
      align: 'right',
      width: 130,
      render: (value: number) => money(value),
    ...
    {
      title: t('product.transfer.remainingSource', 'Source left'),
    ...
    {
      title: t('product.transfer.targetQuantity', 'Target qty'),
  ...
  const columns: ProColumns<RetailTransfer>[] = [
    {
      title: t('product.transfer.search', 'Search'),
      dataIndex: 'search',
      hideInTable: true,
      valueType: 'text',
    },
    {
      title: t('product.transfer.transferNo', 'Transfer no.'),
      dataIndex: 'transfer_no',
      width: 150,
      fixed: 'left',
    },
    {
      title: t('product.transfer.date', 'Date'),
    ...
    {
      title: t('product.transfer.totalCost', 'Total cost'),
    ...
    {
      title: t('product.transfer.status', 'Status'),
      dataIndex: 'status',
      valueType: 'select',
      width: 110,
      valueEnum: {
        posted: { text: t('product.transfer.posted', 'Posted') },
        voided: { text: t('product.transfer.voided', 'Voided') },
      },
      render: (_, record) => (
    ...
    {
      title: t('product.transfer.note', 'Note'),
      dataIndex: 'note',
      search: false,
      ellipsis: true,
    },
    ...
    <PageContainer>
      <ProTable<RetailTransfer>
        ...
        toolBarRender={() => [
          <Statistic
            key="qty"
            title={t('product.transfer.loadedQuantity', 'Loaded qty')}
            value={totalQuantity}
            precision={2}
          />,
          <Statistic
            key="cost"
            title={t('product.transfer.loadedCost', 'Loaded cost')}
            value={totalCost}
            precision={2}
          />,
          <Button key="reload-lookups" icon={<ReloadOutlined />} onClick={reloadLookups}>
            {t('product.transfer.reloadStock', 'Reload stock')}
          </Button>,
          <Button key="new" type="primary" icon={<PlusOutlined />} onClick={openNew}>
            {t('product.transfer.newTransfer', 'New transfer')}
          </Button>,
        ]}
      />
      ...
      <Modal
        title={
          <Space>
            <SwapOutlined />
            <span>{t('product.transfer.newTransfer', 'New transfer')}</span>
          </Space>
        }
        open={open}
      ...
      >
        ...
        >
          <Form.Item
            name="from_lot_id"
            label={t('product.transfer.sourceLot', 'Source stock lot')}
            rules={[{ required: true }]}
          >
          ...
          <Form.Item
            ...
            label={t('product.transfer.toSite', 'To site')}
            rules={[{ required: true }]}
          >
            <Select showSearch optionFilterProp="label" options={siteOptions} />
          </Form.Item>
          <Form.Item name="from_branch_id" label={t('product.transfer.fromBranch', 'From branch')}>
            <Select allowClear showSearch optionFilterProp="label" options={branchOptions} />
          </Form.Item>
          <Form.Item name="to_branch_id" label={t('product.transfer.toBranch', 'To branch')}>
            <Select allowClear showSearch optionFilterProp="label" options={branchOptions} />
          </Form.Item>
          <Form.Item
            name="quantity"
            label={t('product.transfer.quantity', 'Quantity')}
            rules={[{ required: true }]}
          >
            <InputNumber min={0.01} precision={2} style={{ width: '100%' }} />
          </Form.Item>
          <Form.Item name="branch_ref" label={t('product.transfer.branchRef', 'Branch ref')}>
            <Input maxLength={128} />
          </Form.Item>
          <Form.Item name="note" label={t('product.transfer.note', 'Note')}>
