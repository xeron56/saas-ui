...
import { listPOSSetupRecords } from '../Setup/service';
import type { POSSetupRecord } from '../Setup/types';
...
const setupBranchLabel = (record?: POSSetupRecord) =>
...
const StaffUsersPage: React.FC = () => {
  ...
  const [branches, setBranches] = useState<POSSetupRecord[]>([]);
  ...
  useEffect(() => {
    let active = true;
    listPOSSetupRecords('branches', { status: 'active' })
      .then((resp) => {
        if (active) {
