...
const StockPage: React.FC = () => {
  ...
  const handleAdjust = async (values: AdjustmentForm) => {
    ...
    message.success(t('product.stock.adjusted', 'Stock adjusted'));
  ...
  const columns: ProColumns<CatalogStockLot>[] = [
    {
      title: t('product.stock.search', 'Search'),
      dataIndex: 'search',
      hideInTable: true,
      valueType: 'text',
    },
    {
      title: t('product.stock.item', 'Item'),
      dataIndex: 'item_name',
      width: 220,
      render: (_, record) => (
    ...
    {
      title: t('product.stock.type', 'Type'),
      dataIndex: 'item_kind',
      width: 110,
      valueEnum: {
        single: { text: t('product.stock.single', 'Single') },
        variant: { text: t('product.stock.variant', 'Variant') },
        bundle: { text: t('product.stock.bundle', 'Bundle') },
      },
      render: (_, record) => {
    ...
    {
      title: t('product.stock.batch', 'Batch'),
      dataIndex: 'batch_code',
      search: false,
      width: 130,
    },
    {
      title: t('product.stock.variantName', 'Variant'),
      dataIndex: 'variant_name',
      search: false,
      width: 140,
    },
    {
      title: t('product.stock.site', 'Store'),
      dataIndex: ['storage_site', 'label'],
      search: false,
      width: 150,
    },
    {
      title: t('product.stock.quantity', 'Lot qty'),
      dataIndex: 'quantity',
      search: false,
      align: 'right',
      width: 110,
    ...
    {
      title: t('product.stock.itemQuantity', 'Item qty'),
      ...
      render: (_, record) => (
        <Space>
          ...
          {record.low_stock && <Tag color="red">{t('product.stock.low', 'Low')}</Tag>}
        </Space>
      ),
    },
    {
      title: t('product.stock.purchaseCost', 'Cost'),
    ...
    {
      title: t('product.stock.salePrice', 'Sale'),
    ...
    {
      title: t('product.stock.stockValue', 'Value'),
    ...
    {
      title: t('product.stock.expireDate', 'Expiry'),
      ...
      render: (_, record) => (
        <Space>
          ...
          {record.expired && <Tag color="volcano">{t('product.stock.expired', 'Expired')}</Tag>}
        </Space>
      ),
    },
    {
      title: t('product.stock.active', 'Active'),
      dataIndex: 'active',
      valueType: 'select',
      valueEnum: {
        true: { text: t('product.stock.active', 'Active') },
        false: { text: t('product.stock.inactive', 'Inactive') },
      },
      render: (_, record) => (
        <Tag color={record.active === false ? 'default' : 'green'}>
          {record.active === false
            ? t('product.stock.inactive', 'Inactive')
            : t('product.stock.active', 'Active')}
        </Tag>
      ),
      width: 110,
    },
    {
      title: t('product.stock.operate', 'Operate'),
      valueType: 'option',
      width: 110,
      render: (_, record) => [
        <Button
  ...
  return (
    <PageContainer>
      <ProTable<CatalogStockLot>
        ...
        toolBarRender={() => [
          <Segmented
            ...
            options={[
              { label: t('product.stock.all', 'All stock'), value: 'all' },
              { label: t('product.stock.lowStock', 'Low stock'), value: 'low' },
              { label: t('product.stock.expiredStock', 'Expired'), value: 'expired' },
            ]}
          />,
          <Statistic
            key="qty"
            title={t('product.stock.totalQuantity', 'Total quantity')}
            value={quantity}
            precision={2}
          />,
          <Statistic
            key="value"
            title={t('product.stock.stockValue', 'Stock value')}
            value={stockValue}
            precision={2}
          />,
          <Statistic
            key="low"
            title={t('product.stock.lowStock', 'Low stock')}
            value={lowStockCount}
            prefix={<WarningOutlined />}
          />,
          <Statistic
            key="expired"
            title={t('product.stock.expiredStock', 'Expired')}
            value={expiredCount}
          />,
          <Button
            key="reload"
      ...
      <Modal
        open={!!adjusting}
        title={adjusting?.item_name || t('product.stock.adjust', 'Adjust')}
        onCancel={() => {
          setAdjusting(undefined);
          form.resetFields();
        }}
      ...
      >
        <Form<AdjustmentForm> form={form} layout="vertical" onFinish={handleAdjust}>
          <Form.Item
            name="delta"
            label={t('product.stock.adjustDelta', 'Adjustment quantity')}
            rules={[{ required: true }]}
          >
