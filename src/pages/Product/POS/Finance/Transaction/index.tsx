...
const MoneyMovementPage: React.FC = () => {
  ...
  const handleSubmit = async (values: MoneyMovementRequest) => {
    ...
    try {
      ...
      message.success(t('product.finance.movementPosted', 'Movement posted'));
      setEditorOpen(false);
  ...
  const columns: ProColumns<MoneyMovement>[] = [
    ...
    {
      title: t('product.finance.movementNo', 'Movement no.'),
      dataIndex: 'movement_no',
      width: 150,
      render: (_, record) => <a onClick={() => openDetail(record)}>{record.movement_no}</a>,
    },
    {
      title: t('product.finance.account', 'Account'),
    ...
    {
      title: t('product.finance.movementType', 'Movement type'),
      dataIndex: 'movement_type',
      valueEnum: {
        bank_to_bank: { text: t('product.finance.bankToBank', 'Bank to bank') },
        bank_to_cash: { text: t('product.finance.bankToCash', 'Bank to cash') },
        cash_to_bank: { text: t('product.finance.cashToBank', 'Cash to bank') },
        adjust_bank: { text: t('product.finance.adjustBank', 'Adjust bank') },
        adjust_cash: { text: t('product.finance.adjustCash', 'Adjust cash') },
    ...
    },
    {
      ...
      valueEnum: {
        ...
        transfer: { text: t('product.finance.transfer', 'Transfer') },
      },
      width: 120,
    },
    {
  ...
  return (
    <PageContainer>
        ...
        toolBarRender={() => [
          isDayBook && (
            <Tag key="day-book" color="blue">
              {t('product.finance.dayBook', 'Day Book')}
            </Tag>
          ),
          <Statistic
            key="cash"
            title={t('product.finance.cashBalance', 'Cash balance')}
            value={cashBalance}
            precision={2}
          />,
          <Button
          ...
          >
            {t('product.finance.newMovement', 'New money movement')}
          </Button>,
        ]}
      />
      <TransactionEditor
      ...
      <Drawer
        width={760}
        open={detailOpen}
        title={current?.movement_no || t('product.finance_transactions', 'Money movements')}
        onClose={() => setDetailOpen(false)}
        destroyOnClose
      >
        <Descriptions column={2} bordered size="small">
          <Descriptions.Item label={t('product.finance.movementType', 'Movement type')}>
            {current?.movement_type}
          </Descriptions.Item>
          <Descriptions.Item label={t('product.finance.amount', 'Amount')}>
            {money(current?.amount)}
          </Descriptions.Item>
          <Descriptions.Item label={t('product.finance.fromAccount', 'From account')}>
            {current?.from_account?.account_name || '-'}
          </Descriptions.Item>
          <Descriptions.Item label={t('product.finance.toAccount', 'To account')}>
            {current?.to_account?.account_name || '-'}
          </Descriptions.Item>
          <Descriptions.Item label={t('product.finance.account', 'Account')}>
