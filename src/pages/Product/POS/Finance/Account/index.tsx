...
const MoneyAccountPage: React.FC = () => {
  ...
  const handleSubmit = async (values: MoneyAccountRequest) => {
    ...
    try {
      ...
      message.success(t('product.finance.accountSaved', 'Account saved'));
      setEditorOpen(false);
  ...
  const columns: ProColumns<MoneyAccount>[] = [
    ...
    {
      title: t('product.finance.accountName', 'Account name'),
    ...
    {
      title: t('product.finance.branchRef', 'Branch ref'),
      dataIndex: 'branch_ref',
      search: false,
      width: 150,
    },
    {
      title: t('product.finance.openingBalance', 'Opening balance'),
    ...
    {
      title: t('product.finance.currentBalance', 'Current balance'),
    ...
    {
      title: t('product.finance.active', 'Active'),
      dataIndex: 'active',
      valueEnum: {
        true: { text: t('product.finance.active', 'Active') },
        false: { text: t('product.finance.inactive', 'Inactive') },
      },
      render: (_, record) => (
        <Tag color={record.active === false ? 'default' : 'green'}>
          {record.active === false
            ? t('product.finance.inactive', 'Inactive')
            : t('product.finance.active', 'Active')}
        </Tag>
      ),
    },
    {
      title: t('product.finance.updated', 'Updated'),
    ...
    },
    {
      ...
      render: (_, record) => [
        ...
        <Popconfirm
          key="delete"
          title={t('product.finance.deleteAccount', 'Delete this account?')}
          onConfirm={async () => {
    ...
    <PageContainer>
      <ProTable<MoneyAccount>
        ...
        toolBarRender={() => [
          <Statistic
            key="balance"
            title={t('product.finance.totalBankBalance', 'Total bank balance')}
            value={totalBalance}
            precision={2}
          />,
          <Button
          ...
          >
            {t('product.finance.newAccount', 'New payment account')}
          </Button>,
        ]}
      />
      <AccountEditor
