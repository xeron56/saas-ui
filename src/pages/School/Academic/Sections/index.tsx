...
const TableList: React.FC = () => {
  ...
  const [currentRow, setCurrentRow] = useState<V1Section | undefined | null>(undefined);
  const intl = useIntl();

  const handleAdd = async (fields: V1CreateSectionRequest) => {
    const hide = message.loading(
      intl.formatMessage({ id: 'common.creating', defaultMessage: 'Creating...' }),
    );
    try {
      await service.sectionServiceCreateSection({ body: fields });
      hide();
      message.success(
  ...
  const handleUpdate = async (fields: SectionServiceUpdateSectionRequest) => {
    const hide = message.loading(
      intl.formatMessage({ id: 'common.updating', defaultMessage: 'Updating...' }),
    );
    try {
      await service.sectionServiceUpdateSection2({ body: fields, sectionId: currentRow!.id! });
      hide();
      message.success(
  ...
  const handleRemove = async (selectedRow: V1Section) => {
    const hide = message.loading(
      intl.formatMessage({ id: 'common.deleting', defaultMessage: 'Deleting...' }),
    );
    try {
      await service.sectionServiceDeleteSection({ id: selectedRow.id! });
      hide();
      message.success(
  ...
  const columns: ProColumnType<V1Section>[] = [
    {
  ...
  const getData = requestTransform<V1Section, V1SectionFilter>(async (req) => {
    const resp = await service.sectionServiceListSection2({ body: req });
    return resp.data;
  });
  ...
  return (
    <PageContainer>
      ...
      >
        {currentRow?.id && (
          <ProDescriptions<V1Section>
            column={1}
            title={currentRow?.code}
            request={async () => {
              const resp = await service.sectionServiceGetSection({ id: currentRow.id! });
      ...
      <UpdateForm
        onSubmit={async (value) => {
          const success = currentRow
            ? await handleUpdate({ section: value })
