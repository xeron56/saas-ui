import { DownloadOutlined, EyeOutlined, PrinterOutlined, SearchOutlined } from '@ant-design/icons';
import { FormattedMessage, useIntl } from '@umijs/max';
import { Button, Empty, Form, Input, Space, Spin, Table, Tag, Typography, message } from 'antd';
...
const PublicExamResults: React.FC = () => {
  const intl = useIntl();
  const [form] = Form.useForm<{ admissionNo: string }>();
  ...
  const loadCard = useCallback(
    async (row?: V1ExamResultCardSummary, lookupAdmissionNo = admissionNo) => {
      if (!row?.examId || !lookupAdmissionNo) {
      ...
      try {
        const resp =
          await examStudentService.examStudentServiceGetPublishedExamResultCardByAdmissionNo({
            examId: row.examId,
            query: { admissionNo: lookupAdmissionNo },
          });
        setMarksheet(resp.data);
      } catch (error) {
        message.error(
          intl.formatMessage({
            id: 'school.public.examResults.loadFailed',
  ...
  const searchResults = useCallback(
    async (values: { admissionNo: string }) => {
      const nextAdmissionNo = values.admissionNo.trim();
      if (!nextAdmissionNo) {
        return;
      }
      setAdmissionNo(nextAdmissionNo);
      setSelected(undefined);
      setMarksheet(undefined);
      setLoadingList(true);
      try {
        const resp =
          await examStudentService.examStudentServiceListPublishedExamResultCardsByAdmissionNo({
            query: { admissionNo: nextAdmissionNo, pageSize: 100 },
          });
        const nextItems = resp.data.items || [];
        setItems(nextItems);
        await loadCard(nextItems[0], nextAdmissionNo);
      } catch (error) {
        message.error(
          intl.formatMessage({
            id: 'school.public.examResults.listFailed',
  ...
  const downloadMarksheetPDF = async () => {
    if (!selected?.examId || !admissionNo) {
      return;
    }
    try {
      const resp =
        await examStudentService.examStudentServiceDownloadPublishedExamMarksheetPdfByAdmissionNo({
      ...
      message.error(
        intl.formatMessage({
          id: 'school.public.examResults.downloadFailed',
    ...
    <div style={{ minHeight: '100vh', background: '#f5f7fb', padding: 24 }}>
      <style>
        {`@media print {
          ...
          .public-exam-result-print, .public-exam-result-print * { visibility: visible; }
          .public-exam-result-print { position: absolute; left: 0; top: 0; width: 100%; }
          .public-exam-result-controls, .public-exam-result-table { display: none !important; }
        }`}
      </style>
      <Space
        direction="vertical"
      ...
      >
        <Space direction="vertical" size={4}>
          <Typography.Title level={2} style={{ marginBottom: 0 }}>
            <FormattedMessage id="school.public.examResults.title" defaultMessage="Exam Results" />
          </Typography.Title>
          <Typography.Text type="secondary">
            <FormattedMessage
              id="school.public.examResults.subtitle"
              defaultMessage="Search published result cards by admission number."
            />
          </Typography.Text>
        </Space>
        ...
        <Form
          form={form}
          layout="inline"
          className="public-exam-result-controls"
        ...
        >
            ...
            rules={[
              {
                ...
                message: intl.formatMessage({
                  id: 'school.public.examResults.admissionRequired',
                  defaultMessage: 'Enter an admission number.',
                }),
              },
            ]}
          >
          ...
          <Form.Item>
            <Button
              ...
              disabled={!selected?.examId || !admissionNo}
        ...
        <Table<V1ExamResultCardSummary>
          className="public-exam-result-table"
        ...
        <Spin spinning={loadingCard}>
          {marksheet ? (
            <div
              className="public-exam-result-print"
              style={{
                background: '#fff',
            ...
            <Empty
              ...
              description={intl.formatMessage({
                id: 'school.public.examResults.empty',
                defaultMessage: 'Enter an admission number to view published exam results.',
              })}
            />
          )}
        </Spin>
